...
public class CommitBuilder extends JFrame {
	 ...
	 * @Override public void valueChanged(GraphSelectionEvent e) { if (e.getCell() instanceof DefaultGraphCell &&
	 * ((DefaultGraphCell) e.getCell()).getUserObject() instanceof Commit) { Commit c = (Commit) ((DefaultGraphCell)
	 * e.getCell()).getUserObject(); try { jsonPanel.setJson(c.getElement().toString(), UpdateType.REPLACE); } catch
	 * (JsonSyntaxException | StoreException | VisitorException e1) { // TODO Auto-generated catch block
	 * e1.printStackTrace(); } } } });
	 */
	pack();
	new Thread() {
    ...
    public static void main(String args[]) throws JsonSyntaxException, PatchFailedException, StoreException,
	    IOException, InterruptedException, MergeException, CorruptTreeException {
	new CommitBuilder();
    }
