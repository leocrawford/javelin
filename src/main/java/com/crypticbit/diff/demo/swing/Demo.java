	...
	public void onFrameAction(Action action, JFrame frame) {
		if (theFrame == frame) {
			...
			if (action.equals(Action.OK)) {
				JOptionPane.showMessageDialog(null, json, "Resulting JSON",
						JOptionPane.INFORMATION_MESSAGE);
			} else if (action.equals(Action.DEFAULT_CLOSE)) {
				JOptionPane.showMessageDialog(null,
						"The default close button was clicked",
						"User Hit Default Close",
						JOptionPane.INFORMATION_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(null,
						"The cancel button was clicked", "User Hit Cancel",
						JOptionPane.INFORMATION_MESSAGE);
			}
		}
	}
