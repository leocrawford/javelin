...
public class JSONEditFrame extends JFrame implements TreeSelectionListener {
	...
	public JSONEditFrame(OKCancelListener parent, String title,
		...
		for (int loop = 0; loop < 3; loop++) {
			...
			treeView[loop].setJson(initialJson,
		...
		JButton button = new JButton(new CopyJsonAction(Direction.REPLACE,
				treeView, jsonTextArea));
		buttonPanel.add(button);
		treeChangeButtons.put(JSONEditPanel.AllowedOps.REPLACE, button);
		buttonPanel.add(Box.createRigidArea(new Dimension(10, 0)));

		button = new JButton(new CopyJsonAction(Direction.INSERT, treeView,
				jsonTextArea));
		...
		button = new JButton(new CopyJsonAction(Direction.APPEND, treeView,
				jsonTextArea));
		...
		button = new JButton(new CopyJsonAction(Direction.AS_CHILD, treeView,
				jsonTextArea));
		...
		button = new JButton(new CopyJsonAction(Direction.RENAME, treeView,
				jsonTextArea));
		buttonPanel.add(button);
		...
		button = new JButton(new CopyJsonAction(Direction.DELETE, treeView,
				jsonTextArea));
		buttonPanel.add(button);
		...
		button = new JButton(new CopyJsonAction(Direction.GET, treeView,
				jsonTextArea));
	...
	private void updateButtonStates() {
		...
		// operations
		List<JSONEditPanel.AllowedOps> allowedOps = treeView[currentIndex]
	...
	private class CopyJsonAction extends AbstractAction {
		...
		private final ThreeWayDiff<JsonElement> twd = new ThreeWayDiff<>(
				parser.parse("{}"), new DifferFactory() {
					{
		...
		public CopyJsonAction(Direction direction, JSONEditPanel[] treeView,
				JTextArea jTextArea) {
		...
		public void actionPerformed(ActionEvent e) {
			switch (direction) {
			case AS_CHILD:
				jEditPanel[currentIndex].setJson(jTextArea.getText(),
			...
			jEditPanel[2]
					.setJson(
							mergeJson(jEditPanel[currentIndex].getJson(),
									currentIndex),
							JSONEditPanel.UpdateType.REPLACE);
		}
