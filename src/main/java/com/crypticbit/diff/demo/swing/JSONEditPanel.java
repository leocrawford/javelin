...
/**
 * Implements the embeddable swing widget for editing JSON. This class is not
 * thread safe.
 * 
...
public class JSONEditPanel extends JPanel {
	...
	// Default visibility to let classes in this module access this element
	// directly.
	JTree jTree;
	...
	/**
	 * Deletes selected node or sets entire model to null if root node or no
	 * node is selected.
	 */
	public void deleteNode() {
		...
		if (selection == null) {
			...
			jTree.setModel(new DefaultTreeModel(new JSONJTreeNode(null, -1,
					new JsonNull())));
		} else {
			JSONJTreeNode node = (JSONJTreeNode) selection
	...
	/**
	 * Determine what can currently be asked of the component in terms of tree
	 * modifiation operations.
	 * 
	 * @return a list of AllowedOps enumerations to indicate the commands that
	 *         are currently allowed upon the Component
	 */
	public List<AllowedOps> getAllowedOperations() {
		...
		JSONJTreeNode selectedNode = (JSONJTreeNode) selection
				.getLastPathComponent();
		...
		if ((parentNode != null)
				&& (parentNode.dataType.equals(JSONJTreeNode.DataType.OBJECT))) {
	 ...
	 * 
	 * @return the JSON as it is represented by the current state of the Tree
	 *         model
	 */
	...
	public void renameNode() {
		...
		if (selection == null) {
			JOptionPane.showMessageDialog(this,
					"No node is selected for rename.", "Notice",
		...
		if (parent == null) {
			JOptionPane.showMessageDialog(this,
					"It is not possible to assign a name to the root node.",
					"Notice", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
		if (!parent.dataType.equals(JSONJTreeNode.DataType.OBJECT)) {
			JOptionPane.showMessageDialog(this,
					"Only object fields may be renamed.", "Notice",
					JOptionPane.INFORMATION_MESSAGE);
			return;
		}

		String newName = JOptionPane.showInputDialog("Enter new name for "
				+ node.fieldName);
	...
	/**
	 * Replaces the current tree structure in the contained JTree component with
	 * a new structure built from the JSON string provided.
	 * 
	 * @param json
	 *            - the JSON to update the tree control with
	 * @param updateType
	 *            - if a node is selected when this method is called then the
	 *            updateType indicates where the new json goes: REPLACE -
	 *            replace current selected node with new JSON INSERT - place
	 *            node before selected node APPEND - place node after selected
	 *            node AS_CHILD - append to end of child nodes or insert new
	 *            child node if no children present. Selected node must be of
	...
	public void setJson(String json, UpdateType updateType) {
		...
		if (selection == null) {
			...
			case REPLACE: {
				...
				JsonElement root = new JsonParser().parse(json);
				JSONJTreeNode replacementNode = new JSONJTreeNode(
						selectedNode.fieldName, selectedNode.index, root);
			...
			case APPEND: {
				if (parent == null) {
					JOptionPane.showMessageDialog(this,
							"You cannot append to the root element.", "Notice",
							JOptionPane.INFORMATION_MESSAGE);
					return;
				}
				JsonElement root = new JsonParser().parse(json);
			...
			case AS_CHILD: {
				...
				if (selectedNode.dataType.equals(JSONJTreeNode.DataType.ARRAY)) {
					...
					arrayIndex = count;
				} else if (selectedNode.dataType
						.equals(JSONJTreeNode.DataType.OBJECT)) {
					fieldName = "new-field";
				} else {
					JOptionPane.showMessageDialog(this,
							"Vaue type entities can not have children.",
							"Notice", JOptionPane.INFORMATION_MESSAGE);
					return;
				}
				JSONJTreeNode newNode = new JSONJTreeNode(fieldName,
						arrayIndex, root);
