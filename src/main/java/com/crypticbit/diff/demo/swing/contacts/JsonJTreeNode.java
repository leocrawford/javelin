...
/**
 * <p>
 * Provides the model for translating JsonElement into JTree data nodes. This
...
public class JsonJTreeNode extends DefaultMutableTreeNode {
	...
	public JsonJTreeNode(String fieldName, int index, JsonElement jsonElement) {
		...
		if (jsonElement.isJsonArray()) {
			...
			throw new IllegalArgumentException(
					"jsonElement is an unknown element type.");
		}
	...
	public JsonElement asJsonElement() {
		...
		if (json.startsWith("{") || json.startsWith("[")) {
			...
			try {
				...
				Iterator<Entry<String, JsonElement>> it = obj.entrySet()
						.iterator();
	...
	private void populateChildren(JsonElement myJsonElement) {
		...
		case ARRAY:
			int index = 0;
			Iterator<JsonElement> it = myJsonElement.getAsJsonArray()
					.iterator();
			while (it.hasNext()) {
				JsonElement element = it.next();
				JsonJTreeNode childNode = new JsonJTreeNode(null, index,
						element);
				this.add(childNode);
				index++;
			}
			break;
		case OBJECT:
			for (Entry<String, JsonElement> entry : myJsonElement
					.getAsJsonObject().entrySet()) {
				JsonJTreeNode childNode = new JsonJTreeNode(entry.getKey(), -1,
						entry.getValue());
				this.add(childNode);
			}
			break;
