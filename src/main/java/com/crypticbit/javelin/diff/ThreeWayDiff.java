...
/**
 * Performa three way diff for a range of inbuilt types, which can be extended
 * by adding them to the DifferFactory
 */
public class ThreeWayDiff<T> {
	...
	public ThreeWayDiff(T commonAncestor, DifferFactory applicatorFactory) {
		if (LOG.isLoggable(Level.FINER)) {
			LOG.log(Level.FINER, "Created ThreeWayDiff for ancestor"
					+ commonAncestor);
		}
	...
	private SequenceDiff<T, ?> getPatch() {
		if (LOG.isLoggable(Level.FINE)) {
			LOG.log(Level.FINE, "Creating patch for " + commonAncestor + "-> "
					+ list.toString());
		}

		List<Snapshot<T>> workingList = list;
		SequenceDiff<T, ?> result = applicatorFactory
				.createApplicator(commonAncestor);
