		...
		try {
			...
			for (Delta d : Lists.reverse(patch.getDeltas())) {
				...
				if (d.getType() != Delta.TYPE.CHANGE) {
					...
					for (int loop = 0; loop < d.getOriginal().size()
							&& loop < d.getRevised().size(); loop++) {
						int key = d.getOriginal().getPosition() + loop;
						Object o = branchView.get(key);
						// we use the set rather than <code>instanceof</code>
						// because it is legal to add any type of
						// object, including ThreeWayDiff.
						if (!recursiveDiffs.contains(o)) {
							// FIXME copy state into new three way diff if
							// required
							ThreeWayDiff twd = new ThreeWayDiff(d.getOriginal()
									.getLines().get(loop));
							branchView.set(key, twd);
							recursiveDiffs.add(twd);
							o = twd;
						}
						((ThreeWayDiff) o).addBranchSnapshot(d.getRevised()
								.getLines().get(loop), branch);

					}
					// if the original was longer..
					for (int loop = d.getRevised().size(); loop < d
							.getOriginal().size(); loop++) {
						branchView.remove(loop + d.getOriginal().getPosition());
					}
					// if the original was shorter..
					for (int loop = d.getOriginal().size(); loop < d
							.getRevised().size(); loop++) {
						branchView.add(loop + d.getOriginal().getPosition(), d
								.getRevised().getLines().get(loop));
					}
