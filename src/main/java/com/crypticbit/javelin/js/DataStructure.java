...
public class DataStructure {
	...
	/**
	 * Create a new json data structure - by selecting a label from an existing
	 * ds by name
	 * 
	 * @throws Error
	 * @throws VisitorException
	...
	private DataStructure(DataStructure parent) throws StoreException {
		...
		this.selectedAnchor = new ExtendedAnchor<>(store,
				parent.selectedAnchor, CommitDao.class);
	...
	public void exportAll(OutputStream outputStream)
		 ...
		 * Set<Key> tempCas = new HashSet<>(); Set<Key> tempKas = new
		 * HashSet<>(); Map<String, Key> labelToCommitMap = new HashMap<>();
		 * LabelsDao labels = labelsAnchor.readEndPoint(store); for (String
		 * label : labels.getLabels()) { System.out.println("Processing label "
		 * + label); Key commitAddress = labels.getCommitAnchor(label,
		 * jsonFactory).getAddress(); tempKas.add(commitAddress); CommitDao
		 * commitDao = labels.getCommitAnchor(label,
		 * jsonFactory).readEndPoint(store); Key destination =
		 * labels.getCommitAnchor(label, jsonFactory).read(store);
		 * System.out.println("dest=" + destination + "," + commitDao);
		 * labelToCommitMap.put(label, destination); Commit c = new
		 * Commit(commitDao, destination, jsonFactory); DirectedGraph<Commit,
		 * DefaultEdge> x = c.getAsGraphToRoot(); for (Commit v : x.vertexSet())
		 * { tempCas.addAll(v.getAllIdentities()); System.out.println("Commit="
		 * + v.getIdentity2() + "," + v.toString() + "," +
		 ...
		 * System.out.println("cas: " + tempCas); System.out.println("kas: " +
		 * tempKas); System.out.println("all: " + store.list()); Set<Key>
		 * missing = new HashSet(store.list()); missing.removeAll(tempCas);
		 * missing.removeAll(tempKas); for (Key i : missing) {
		 * System.out.println(i + "=" + store.getCas(i)); } Map<Key,
		 * PersistableResource> casRresult = new HashMap<>(); for (Key i :
		 * tempCas) { casRresult.put(i, store.getCas(i)); }
		 * oos.writeObject(casRresult); Map<Key, PersistableResource> kasResult
		 * = new HashMap<>(); for (Key i : tempKas) { kasResult.put(i,
		 * store.getCas(i)); } oos.writeObject(kasResult);
		 */
	...
	public void importAll(InputStream inputStream, MergeType mergeType)
		 ...
		 * ois.readObject(); LabelsDao localLabels =
		 * labelsAnchor.readEndPoint(store); // copy all cas elements Map<Key,
		 * PersistableResource> casResult = (Map<Key, PersistableResource>)
		 * ois.readObject(); for (Entry<Key, PersistableResource> x :
		 * casResult.entrySet()) { Key idOfValueWrittenToStore =
		 * store.store(x.getValue()); if
		 * (!idOfValueWrittenToStore.equals(x.getKey())) { throw new
		 * IllegalStateException("The entry " + x.getKey() +
		 * " produced a new key on store to local (" + idOfValueWrittenToStore +
		 * ")"); } }
		 */
		/*
		 * // copy all kas elements Map<Identity, PersistableResource> kasResult
		 * = (Map<Identity, PersistableResource>) ois.readObject(); for
		 * (Entry<Identity, PersistableResource> x : kasResult.entrySet()) {
		 * System.out.println("Wrote kas: " + x.getKey()); // FIXME - what
		 * should I check against? // FIXME store.get(x.getKey()).getBytes())
		 * very cumbersome if (store.check(x.getKey())) { if (mergeType ==
		 * MergeType.OVERWRITE) { System.out.println("Over-writing:" +
		 * x.getKey()); store.store(x.getKey(), new
		 * Digest(store.get(x.getKey()).getBytes()), new Digest(x.getValue()
		 * .getBytes())); } else { // throw new
		 * StoreException("KAS Conflict in " + mergeType + " mode. Key = " +
		 * x.getKey()); // fFIXME probably wnat to ignore some of these..
		 * System.out.println("KAS Conflict in " + mergeType + " mode. Key = " +
		 * x.getKey() + "goes to "+store.get(x.getKey())+" but incoming "+new
		 * Digest(x.getValue() .getBytes())); } } else store.store(x.getKey(),
		 * null, new Digest(x.getValue().getBytes()));
		...
		// merge labels
		/*
		 * for (String importedLabel : labelToCommitMap.keySet()) { if
		 * (localLabels.hasCommitAnchor(importedLabel)) {
		 * ExtendedAnchor<CommitDao> localCommitAnchor =
		 * localLabels.getCommitAnchor(importedLabel, jsonFactory); switch
		 * (mergeType) { case OVERWRITE: System.out.println("overwrite label " +
		 * importedLabel + " from " + localLabels); // FIXME does deleting read
		 * work? // localCommitAnchor.readEndPoint(store);
		 * localCommitAnchor.write(store, labelToCommitMap.get(importedLabel));
		 * break; case IGNORE_CONFLICT:
		 * System.out.println("Doing nothing with label " + importedLabel +
		 * " from " + localLabels); break; case MERGE:
		 * System.out.println("Merging label " + importedLabel + " from " +
		 * localLabels); localCommitAnchor.readEndPoint(store); try { Commit ic
		 * = new
		 * Commit(jsonFactory.getSimpleObjectAdapter(CommitDao.class).read(
		 * labelToCommitMap.get(importedLabel)),
		 * labelToCommitMap.get(importedLabel), jsonFactory);
		 * System.out.println("import destination=" +
		 * labelToCommitMap.get(importedLabel));
		 * System.out.println("local address=" +
		 * localCommitAnchor.getAddress());
		 * System.out.println("local destination=" +
		 * localCommitAnchor.getDestination()); System.out.println(new
		 * Commit(localCommitAnchor.getEndPoint(), localCommitAnchor
		 ...
		 * labelToCommitMap.get(importedLabel)).getDao()); } catch
		 * (PatchFailedException e) { // FIXME betetr exception handling throw
		 * new StoreException("Can't merge", e); } break; } } else
