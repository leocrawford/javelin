...
/**
 * Merkle tree is a tree in which every non-leaf node is labelled with the hash
 * of the labels of its children nodes. Hash trees are useful because they allow
 * efficient and secure verification of the contents of large data
...
public class MerkleTree {
	...
	/**
	 * Create a new merkle tree with a random anchor, and new set of labels
	 * containing only HEAD (which initially points at nothing)
	 * 
	...
	/**
	 * Create a merkle tree using a store with an existing set of labels, which
	 * are provided by labelsAddress and the current label identified by label
	 */
	public MerkleTree(AddressableStorage store, Key labelsAddress, String label)
			throws JsonSyntaxException, StoreException {
		this(store, new ExtendedAnchor<>(touch(store), labelsAddress,
				LabelsDao.class), label);
	}

	private MerkleTree(AddressableStorage store,
			ExtendedAnchor<LabelsDao> labelsAnchor,
	...
	private MerkleTree(MerkleTree parent) throws StoreException {
		this(parent.store, parent.labelsAnchor, new ExtendedAnchor<>(
				parent.store, parent.selectedAnchor, CommitDao.class));
	}

	private static ExtendedAnchor<LabelsDao> createLabels(
			AddressableStorage store) throws StoreException {
		ExtendedAnchor<LabelsDao> labelsAnchor = new ExtendedAnchor<>(store,
				LabelsDao.class);
	...
	private MerkleTree(AddressableStorage store,
			ExtendedAnchor<LabelsDao> labelsAnchor, String label)
			throws JsonSyntaxException, StoreException {
		this(store, labelsAnchor, labelsAnchor.getDestinationValue()
				.getCommitAnchor(label, touch(store)));
	}
	...
	public synchronized MerkleTree merge(MerkleTree other)
			throws PatchFailedException, MergeException, CorruptTreeException,
			StoreException {
		merge(selectedAnchor,
	...
	public MerkleTree write(String string) throws CorruptTreeException,
			StoreException {
		Key write = commitFactory.getJsonElementStoreAdapter().write(
				gson.fromJson(string, JsonElement.class));
		commitFactory.createCommit(selectedAnchor, write,
				selectedAnchor.getDestinationAddress());
		return this;
	}

	public void write(String path, String json) throws StoreException,
		...
		Key valueIdentity = commitFactory.getObjectStoreAdapter().write(
				originalResult);
		// FIXME patterm of commit = create then checkout repeated several times
		commitFactory.createCommit(selectedAnchor, valueIdentity,
				selectedAnchor.getDestinationAddress());
	}

	private Commit merge(ExtendedAnchor<CommitDao> commitAnchorToMergeA,
			Commit commitB, Key addressB) throws PatchFailedException,
			MergeException, CorruptTreeException, StoreException {
		ThreeWayDiff patch = commitFactory.getCommitFromAnchor(
				commitAnchorToMergeA).createChangeSet(commitB);
		Key valueIdentity = commitFactory.getObjectStoreAdapter().write(
				patch.apply());
		return commitFactory.createCommit(selectedAnchor, valueIdentity,
				commitAnchorToMergeA.getDestinationAddress(), addressB);
	}
	...
	public void sync(AddressableStorage remote, String label, MergeType type)
			throws CorruptTreeException, StoreException, PatchFailedException,
			MergeException {

		// find the local stores anchor to the named label
		ExtendedAnchor<CommitDao> anchor = (MergeType.OVERWRITE == type) ? null
				: labelsAnchor.getDestinationValue().getCommitAnchor(label,
						store);
		try {
		...
		// use a StoreAggregator which allows us to access both stores as if
		// they were one. Everything we access which
		// is only available in the remote store will be copied to th3 local
		// store.
		...
		ExtendedAnchor<CommitDao> remoteAnchor = new ExtendedAnchor<CommitDao>(
				remote, anchor.getSourceAddress(), CommitDao.class);

		// FIXME force load
		aggregateStore.getCas(remoteAnchor.getDestinationAddress(),
				CommitDao.class);
		Commit c = tempFactory.getCommitFromAnchor(remoteAnchor);
		c.recusivelyLoad();

		if (c.getAsGraphToRoot().containsVertex(this.getCommit())) {
			LOG.log(Level.INFO, "Updating " + label
					+ " to newer version from remote");
			anchor.setDestinationAddress(remoteAnchor.getDestinationAddress());
		} else if (this.getCommit().getAsGraphToRoot().containsVertex(c))
			LOG.log(Level.INFO, "Ignoring incoming from remote " + label
					+ " as already in stream");
		else {
	...
	public void sync(AddressableStorage store, String head)
			throws PatchFailedException, CorruptTreeException, StoreException,
			MergeException {
		sync(store, head, MergeType.MERGE);
