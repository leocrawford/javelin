...
/**
 * Provides a memory based CAS. The digest is calculated using a
 * <code>digestFactory</code> which is created at instantiation, and has the
 * same life as this object. You can change the default algorithm used by
...
public class MemoryCasKas implements CasKasStore {
	...
	public PersistableResource get(Identity digest) {
		GeneralPersistableResource pr = new GeneralPersistableResource(
				casMap.get(digest));
		if (LOG.isLoggable(Level.FINEST)) {
			LOG.log(Level.FINEST, "Read " + pr.getBytes().length
					+ " bytes from " + digest + " using memory CAS");
		}
		return pr;
	}
	...
	@Override
	public synchronized void store(Identity id, Identity oldDigest,
			Identity newDigest) throws StoreException {
		if (!check(id) || new Digest(get(id).getBytes()).equals(oldDigest)) {
			...
			throw new StoreException("Concurrent modification. Expected "
					+ oldDigest + " but got " + new Digest(get(id).getBytes()));
		}
	}
	...
	public Identity store(PersistableResource pr) {
		...
		if (LOG.isLoggable(Level.FINEST)) {
			LOG.log(Level.FINEST, "Adding " + pr.getBytes().length
					+ " bytes to " + digest + " in memory CAS");
		}
