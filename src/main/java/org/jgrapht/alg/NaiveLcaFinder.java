/*
 * ========================================== JGraphT : a free Java graph-theory
 * library ==========================================
 * 
 * Project Info: http://org.org.jgrapht.sourceforge.net/ Project Creator: Barak
 * Naveh (http://sourceforge.net/users/barak_naveh)
 * 
 * (C) Copyright 2003-2013, by Barak Naveh and Contributors.
 * 
 * This program and the accompanying materials are dual-licensed under either
 * 
 * (a) the terms of the GNU Lesser General Public License version 2.1 as
 * published by the Free Software Foundation, or (at your option) any later
 * version.
 * 
 * or (per the licensee's choosing)
 * 
 * (b) the terms of the Eclipse Public License v1.0 as published by the Eclipse
 * Foundation.
 */
/*
 * ------------------------- NaiveLcaFinder.java -------------------------
 * 
 * Original Author: Leo Crawford Contributor(s):
 */
...
public class NaiveLcaFinder<V, E> {
	 ...
	 * <p>
	 * Find the LCA, defined as <i>Let G = (V, E) be a DAG, and let x, y ∈ V .
	 * Let G x,y be the subgraph of G induced by the set of all common ancestors
	 * of x and y. Define SLCA (x, y) to be the set of out-degree 0 nodes
	 * (leafs) in G x,y . The lowest common ancestors of x and y are the
	 ...
	 * continues
	 * <p>
	 * This means at the end of recursion n, we know if there is an LCA that has
	 * a path of <=n to a and b. Of course we may have to wait longer if the
	 * path to a is of length n, but the path to b>n. at the first loop we have
	 * a path of 0 length from the nodes we are considering as LCA to their
	 * respective children which we wish to find the LCA for.
	 */
	...
	public V findLca(V a, V b) {
		return findLca(Collections.singleton(a), Collections.singleton(b),
				new LinkedHashSet<V>(), new LinkedHashSet<V>());
	}
	 ...
	 */
	private V findLca(Set<V> aSet, Set<V> bSet, LinkedHashSet<V> aSeenSet,
		...
		// no point doing the same again (and it can stop us getting stuck in an
		// infinite loop)
	 ...
	 * @param y
	 *            set containing vertex, which may be ordered to give
	 *            predictable results
