...
public class Neo4JJsonPersistenceServiceTest extends Neo4JTestSupport {
	...
	ps.getRootNode().getJsonNode().write(JSON_TEXT);
	assertEquals(MAPPER.readTree(JSON_TEXT), MAPPER.readTree(ps.getRootNode().getJsonNode().toJsonString()));
    }
	...
	ps.getRootNode().getJsonNode().write(JSON_TEXT);
	ps.getRootNode().getJsonNode().navigate("second").getJsonNode().write("\"new value\"");

	assertEquals(MAPPER.readTree("{\"first\": 123, \"second\": \"new value\", \"third\": 789, \"xid\": null}"),
		MAPPER.readTree(ps.getRootNode().getJsonNode().toJsonString()));

	ps.getRootNode().getJsonNode().navigate("second").getJsonNode().write("[0,1,2]");
