...
public class MarkleTreeTest extends TestUtils {
	 ...
	 * "[\"foo\",100,{\"a\":1000.21,\"b\":6},true,null,[1,2,3]]"; private static
	 * final String JSON_EXAMPLE_2 =
	 * "[\"foo\",100,{\"a\":1000.21,\"b\":6},true,null,[1,2,3,4]]"; private
	 * static final String JSON_EXAMPLE_3 =
	 * "[\"foo\",100,{\"a\":1000.21,\"b\":6},true,null,[1,2,3,5]]"; private
	 * static final String JSON_EXAMPLE_4 =
	 * "[\"foo\",100,{\"a\":1000.21,\"b\":6},true,null,[1,2,3,7]]"; private
	 * static final String JSON_EXAMPLE_5 =
	 * "[\"foo\",100,{\"a\":1000.21,\"b\":6},true,null,[1,2,3,7,8]]";
	 */
	...
	public void testBasicBranch() throws IOException, StoreException,
			JsonSyntaxException, PatchFailedException, MergeException,
		...
		// JsonParser().parse("[\"a\", {\"b\":FALSE, \"c\":2.1, \"a\":2}]"),
		// jca.read());
	...
	@Test
	public void testCommitForComplexMultipleReadWrite() throws IOException,
			StoreException, JsonSyntaxException, CorruptTreeException {
		// enableLog();
		MerkleTree jca = new MerkleTree(new StorageFactory().createMemoryCas());
		jca.write(JSON_EXAMPLE);
		...
		Assert.assertEquals(new JsonParser().parse(JSON_EXAMPLE).toString(), c3
				.getAsElement().toString());
		Assert.assertEquals(new JsonParser().parse(JSON_EXAMPLE_3).toString(),
				c3.getFirstParent().getFirstParent().getAsElement().toString());
	}

	@Test
	public void testCommitForMultipleReadWrite() throws IOException,
			StoreException, CorruptTreeException {
		enableLog();
		MerkleTree jca = new MerkleTree(new StorageFactory().createMemoryCas());
		...
		Assert.assertEquals(new JsonParser().parse(JSON_EXAMPLE).toString(), c3
				.getShortestHistory().get(2).getAsElement().toString());
		Assert.assertEquals(new JsonParser().parse(JSON_EXAMPLE_3).toString(),
				c3.getShortestHistory().get(0).getAsElement().toString());
	}

	@Test
	public void testConcurrentWriteUsingTwoObjects() throws IOException,
			StoreException, CorruptTreeException {
		enableLog();
	...
	@Test
	public void testImportExport() throws IOException, StoreException,
			JsonSyntaxException, ClassNotFoundException, PatchFailedException,
			InterruptedException, CorruptTreeException, MergeException {

		MemoryAddressableStorage store1 = (MemoryAddressableStorage) new StorageFactory()
				.createMemoryCas();
	...
	public void testMultiplBranches() throws StoreException,
		...
		Assert.assertEquals(new JsonParser().parse(JSON_EXAMPLE_2).toString(),
				new MerkleTree(store, d1.getLabelsAddress(), "Branch1").read()
						.toString());
		Assert.assertEquals(new JsonParser().parse(JSON_EXAMPLE_5).toString(),
				new MerkleTree(store, d1.getLabelsAddress(), "Branch2").read()
						.toString());
		Assert.assertEquals(new JsonParser().parse(JSON_EXAMPLE_4).toString(),
				new MerkleTree(store, d1.getLabelsAddress(), "Branch3").read()
						.toString());

	}

	@Test
	public void testReadWriteUsingTwoObjects() throws IOException,
			StoreException, JsonSyntaxException, CorruptTreeException {
		AddressableStorage store = new StorageFactory().createMemoryCas();
		MerkleTree jca = new MerkleTree(store);
		jca.write(JSON_EXAMPLE);
